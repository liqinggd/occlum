...
impl<'a> DirentBufWriter<'a> {
    ...
    fn try_write64(&mut self, inode: u64, type_: u8, name: &str) -> Result<()> {
    ...
    fn try_write(&mut self, inode: u64, name: &str) -> Result<()> {
        let len = ::core::mem::size_of::<LinuxDirent>() + name.len() + 1;
        let len = (len + 7) / 8 * 8; // align up
        if self.rest_size < len {
            return_errno!(EINVAL, "the given buffer is too small");
        }
        let dent = LinuxDirent {
            ino: inode,
            offset: 0,
            reclen: len as u16,
            name: [],
        ...
        unsafe {
            let ptr = self.buf.as_mut_ptr().add(self.written_size) as *mut LinuxDirent;
            ptr.write(dent);
            let name_ptr = ptr.add(1) as _;
            write_cstr(name_ptr, name);
        }
...
pub fn do_getdents64(fd: FileDesc, buf: &mut [u8]) -> Result<usize> {
    ...
    loop {
        ...
        if let Err(e) = writer.try_write64(0, 0, &name) {
...
pub fn do_getdents(fd: FileDesc, buf: &mut [u8]) -> Result<usize> {
    ...
    loop {
        ...
        // TODO: get ino from dirent, set ino to 1 to avoid entering infinite loop in libc
        if let Err(e) = writer.try_write(1, &name) {
            file_ref.seek(SeekFrom::Current(-1))?;
            if writer.written_size == 0 {
                return Err(e);
            } else {
